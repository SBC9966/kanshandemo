...
function ensureScene3D(){
  ...
  if(ctl)ctl.onUserTakeover=()=>{ctx.ui.scene3dDirector=false;document.querySelectorAll('[data-action="scene3d-director"]').forEach(b=>{b.classList.remove('is-on');b.setAttribute('aria-pressed','false');});toast('已切到自由观察：拖动旋转、滚轮缩放；点罗盘回到自动导览。','compass');};
}
